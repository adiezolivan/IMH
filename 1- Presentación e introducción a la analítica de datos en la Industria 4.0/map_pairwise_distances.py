...
def draw_matrix_of_distances(centroids, labels, njoints=6, weights=None, distance='Euclidean', p=0.1, cov_matrix=None):   
    ...
    x_axis = list()
    map(lambda x: not x in x_axis and x_axis.append(x), labels)     
    #x_axis = labels
    xTickMarks = ['Joint ' + str(int(x_axis[i])) for i in range(len(x_axis))]
    plt.xticks(np.arange(0,len(global_matrix)), xTickMarks, rotation=45)#, fontsize=6)
    plt.yticks(np.arange(0,len(global_matrix)), xTickMarks, rotation=45)#, fontsize=6)
